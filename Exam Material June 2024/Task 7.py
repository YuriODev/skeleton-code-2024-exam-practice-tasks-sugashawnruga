...
class Puzzle():
  ...
  ### CHANGE BELOW ###
  def UndoPreviousMove(self, previousmove):
    last_move_index = len(previousmove._ListofPreviousMoves) - 1
    last_moves = previousmove._ListofPreviousMoves[last_move_index]
    cell = self.__GetCell(last_moves[0], last_moves[1])
    cell.ChangeSymbolInCell(previousmove._ListofPreviousSymbols[last_move_index])
    previousmove._ListofPreviousMoves.pop()
    previousmove._ListofPreviousSymbols.pop()
    print("Great Success")
  ...
  def AttemptPuzzle(self):
    ...
    while not Finished:
      ...
      print(f"You have {len(previousmove._ListofPreviousMoves)} undo moves left.")
      ### CHANGE BELOW ###
      while len(previousmove._ListofPreviousMoves) != 0:
        choice = input("Enter 'y' if you want to undo the previous move: ")
        if choice == 'y':
          self.UndoPreviousMove(previousmove)
          self.DisplayPuzzle()
          print("Current score: " + str(self.__Score))
          print(f"You have {len(previousmove._ListofPreviousMoves)} undo moves left.")
        else:
      ...
      ### CHANGE BELOW ###
      previousmove.storepreviouscellinfo(Row, Column, CurrentCell)
...
class PreviousMove(Cell):
  def __init__(self):
    super().__init__()
    self._ListofPreviousMoves = []
    self._ListofPreviousSymbols = []

  def storepreviouscellinfo(self, Row, Column, C):
    Move = [Row, Column]
    self._ListofPreviousMoves.append(Move)
    self._ListofPreviousSymbols.append(C._Symbol)
